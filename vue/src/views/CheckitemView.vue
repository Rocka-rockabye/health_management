<template>
  <div>
    ...
    <div style="margin-top: 20px">
      <el-input v-model="params.code" style="width: 200px" placeholder="请输入检查项编码"></el-input>
      <el-input v-model="params.name" style="width: 200px; margin-left: 8px" placeholder="请输入检查项名称"></el-input>
      ...
      <el-upload action="http://jkgl.free.idcfengye.com/api/checkitem/upload" :show-file-list="false" style="display: inline-block; margin-left: 10px" :on-success="successUpload">
    ...
    <div>
      <el-table :data="tableData" style="width: 100%; margin: 15px 0px" ref="table" @selection-change="handleSelectionChange" :row-key="getRowKeys">
        ...
        <el-table-column prop="code" label="检查项编码" ></el-table-column>
        <el-table-column prop="name" label="检查项名称" ></el-table-column>
        <el-table-column prop="sex" label="适用性别" ></el-table-column>
        <el-table-column prop="age" label="适用年龄/周岁" ></el-table-column>
        <el-table-column prop="type" label="检查类型" ></el-table-column>
        <el-table-column prop="price" label="检查价格/元" ></el-table-column>
        <el-table-column prop="attention" label="注意事项" ></el-table-column>
        <el-table-column prop="remark" label="检查项说明" ></el-table-column>
    ...
    <div @keyup.enter="submit()">
      <el-dialog title="新增检查项" :visible.sync="dialogFormVisible" width="40%" style="height: 100%">
        <el-form :model="form">
          <el-form-item label="检查项编码" label-width="30%">
            <el-input v-model="form.code" autocomplete="off" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="检查项名称" label-width="30%">
            <el-input v-model="form.name" autocomplete="off" style="width: 90%" placeholder="请输入检查项名称"></el-input>
          </el-form-item>
          ...
          <el-form-item label="适用年龄" label-width="30%">
            <el-input v-model="form.age" autocomplete="off" style="width: 90%" placeholder="请输入数字格式：xx-xx"></el-input>
          </el-form-item>
          <el-form-item label="检查类型" label-width="30%">
            <el-radio v-model="form.type" label="1">检查</el-radio>
            <el-radio v-model="form.type" label="2">检验</el-radio>
          </el-form-item>
          <el-form-item label="检查价格" label-width="30%">
          ...
          <el-form-item label="检查项说明" label-width="30%">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      cancel(){
        if(this.form.code || this.form.name || this.form.sex || this.form.age || this.form.type || this.form.price || this.form.remark || this.form.attention){
          this.form = {};
          this.$message.warning("检查项信息已重置！");
        }else{
      ...
      // 新增提交
      submit() {
        request.post("/checkitem", this.form).then(res => {
          if (res.code === '0') {
            this.$message({
              message: '操作成功',
              type: 'success'
      ...
      // 删除
      del(id) {
        request.delete("/checkitem/" + id).then(res => {
          if (res.code === '0') {
            this.$message({
              message: '删除成功',
              type: 'success'
      ...
      delBatch(){
        ...
        request.put('/checkitem/delBatch',this.multipleSelection).then(res =>{
          if (res.code === '0'){
      ...
      exp(){
        ...
        location.href = 'http://jkgl.free.idcfengye.com/api/checkitem/export?token='+ JSON.parse(user).token
      },
      // 批量导入
      successUpload(res){
        if (res.code === '0'){
